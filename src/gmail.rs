...
mod tests {
    ...
    #[test]
    fn find_calendar_ics_windows_tzid_invite_parses_to_correct_instant() {
        // kata rq9n provider parity: an Exchange-produced invite (Windows
        // timezone name in TZID) arriving through Gmail must survive base64
        // extraction and parse to the DST-correct instant: 16:30 "GMT
        // Standard Time" on Aug 20 is 16:30 BST = 15:30 UTC.
        const WINDOWS_TZID_ICS: &str = "BEGIN:VCALENDAR\r\n\
            ...
            SEQUENCE:0\r\nEND:VEVENT\r\nEND:VCALENDAR";
        let payload = calendar_part(WINDOWS_TZID_ICS);
        let ics = inline_ics(&payload);
    ...
    #[test]
    fn parse_google_datetime_non_utc_offset_normalizes() {
        // Google Calendar returns RFC3339 with an explicit offset; a non-UTC
        // offset (BST here) must normalize to the same instant, matching the
        // coverage the ICS path has for non-UTC zones.
        let block = serde_json::json!({
            "dateTime": "2026-08-20T16:30:00+01:00",
            "timeZone": "Europe/London"
        });
        let parsed = parse_google_datetime(&block).unwrap();
        assert_eq!(parsed.to_rfc3339(), "2026-08-20T15:30:00+00:00");
    }
